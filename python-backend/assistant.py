    ...
    if Planner_Agent:
        planner = Agent(
            ...
            instructions=[
                ...
                "  <logic>",
                "    1. SandboxTools.get_workspace_overview()",
                "    2. SandboxTools.search_code(query) and SandboxTools.read_file(file_path, start_line, end_line) for precise context",
                "    3. SandboxTools.edit_file(...) or SandboxTools.write_file(...) for deterministic edits",
                "    4. SandboxTools.execute_in_sandbox(command) for build/test/verification",
                ...
                "  <methods>",
                "    <method name=\"get_workspace_overview(max_files?)\" returns=\"workspace file list\" />",
                "    <method name=\"search_code(query, path?, file_glob?, case_sensitive?, max_results?)\" returns=\"file:line matches\" />",
                "    <method name=\"read_file(file_path, start_line?, end_line?)\" returns=\"line-numbered text\" />",
                "    <method name=\"write_file(file_path, content)\" returns=\"write status\" />",
                "    <method name=\"edit_file(file_path, search_text, replace_text, replace_all?)\" returns=\"deterministic replacement status\" />",
    ...
    if coding_assistant:
        ...
        dev_team = Agent(
            ...
            instructions=[
                ...
                "Use sandbox tools for file operations, code execution, terminal commands, and testing.",
                "Default workspace root is /home/sandboxuser/workspace. Keep all code files there.",
                "For coding tasks, use this deterministic edit contract: get_workspace_overview -> search_code -> read_file -> edit_file/write_file -> execute_in_sandbox tests.",
                "Avoid full-file rewrites when possible. Prefer edit_file for precise, minimal edits.",
                ...
                "For deployed-site modifications, first use copy_deployed_project(site_id, deployment_id?, target_directory under /home/sandboxuser/workspace), then edit files, then use redeploy_project(site_id, project_directory).",
